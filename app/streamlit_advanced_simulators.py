...
# Sidebar settings
test_type = st.sidebar.selectbox("Choose Test Type", ["A/B Test", "Multivariate Test (ANOVA)", "Multi-arm Test"])
...
if test_type == "A/B Test":
    ...
    for i in range(num_groups):
        ...
        data.extend([(f"Group {i+1}", x) for x in group])

    df = pd.DataFrame(data, columns=["Group", "Converted"])
    model = sm.stats.anova_lm(sm.OLS.from_formula("Converted ~ Group", df).fit(), typ=2)
    st.write(model)
    ...
    with pm.Model():
        alpha = 1
        beta = 1
        p1 = pm.Beta("p1", alpha=alpha + control.sum(), beta=beta + size - control.sum())
        p2 = pm.Beta("p2", alpha=alpha + test.sum(), beta=beta + size - test.sum())
        diff = pm.Deterministic("difference", p2 - p1)
        trace = pm.sample(1000, step=pm.Metropolis(), progressbar=False, return_inferencedata=False)

    st.write(f"Probability that Variant B is better than A: {(trace['difference'] > 0).mean():.2%}")
    ...
    st.metric("Estimated Average Treatment Effect (ATE)", f"{ate:.2f}")
